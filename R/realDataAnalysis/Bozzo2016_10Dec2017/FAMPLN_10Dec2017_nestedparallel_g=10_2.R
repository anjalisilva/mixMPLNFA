...
# testing for G=10, q=1:3, models=(2)
...
if (as.integer(input) == 1){
  G=as.integer(10)
  m=as.integer(2)  # total number of cluster sizes to test 
}
if (as.integer(input) == 2){
  G=as.integer(10)
  m=as.integer(4)  # total number of cluster sizes to test 
}
if (as.integer(input) == 3){
  G=as.integer(10)
  m=as.integer(6)  # total number of cluster sizes to test 
}
if (as.integer(input) == 4){
  G=as.integer(10)
  m=as.integer(8)  # total number of cluster sizes to test 
}
...
# run flat parallelism clustering
results2=list()
results2[[1]]=list()
...
for(c in 1:3){
  results2[[1]][[c]]=list()
  results2[[1]][[c]][[1]] = main_mpln(mod=mod,y=bozzo2016, G=G, n_chain=3, numb_iterations=1000, membership=NA, init_method="kmeans", init_iterations=3, normalize="TMM", modelname=modelnames_consider[m], q=c)
}
save.image(file=paste0("FAMPLN_10Dec2017_nestedparallel_g=",G,"_",m,".RData"))
