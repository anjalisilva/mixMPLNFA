...
results2 <-
  ...
  foreach(m=models_testing, .inorder=FALSE) %dopar% { # for each model
    set.seed(1)
    obj =  main_mpln(mod=mod,y=bozzo2016, G=a, n_chain=3, numb_iterations=1000, membership=NA, init_method="kmeans", init_iterations=3, normalize="TMM", modelname=modelnames_consider[m], q=c)
  }
save.image("FAMPLN_10Dec2017_nestedparallel_g=2_2.RData")
...
#save.image("FAMPLN_07Oct2017_g2q2UUU_nestedparallel_simdata_2.RData")
...
plot(results2[[1]][[1]][[1]]$allresults$loglikelihood[-1], type="l")
plot(results2[[1]][[2]][[1]]$allresults$loglikelihood[-1], type="l")
plot(results2[[1]][[3]][[1]]$allresults$loglikelihood[-1], type="l")


library(coda)
all(heidel.diag(results2[[1]][[1]][[1]]$allresults$loglikelihood[-1], eps=0.1, pvalue=0.05)[,c(1,4)]==1)
all(heidel.diag(results2[[1]][[2]][[1]]$allresults$loglikelihood[-1], eps=0.1, pvalue=0.05)[,c(1,4)]==1)
all(heidel.diag(results2[[1]][[3]][[1]]$allresults$loglikelihood[-1], eps=0.1, pvalue=0.05)[,c(1,4)]==1)
