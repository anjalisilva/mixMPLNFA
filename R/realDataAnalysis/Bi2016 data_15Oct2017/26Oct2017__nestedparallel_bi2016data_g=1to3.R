...
BIC_function<-function(ll, k, n, run, qmin, qmax, gmin, gmax, modelnames, models){
  ...
  BICmodel_labels = run[[which(min(BICvalues) == BICvalues, arr.ind = T)[3]]][[which(min(BICvalues) == BICvalues, arr.ind = T)[1]]][[which(min(BICvalues) == BICvalues, arr.ind = T)[2]]][[1]]$allresults$clusterlabels # obtaining model labels
  BICMessage = NA
...
ICL_function<-function(bIc, run, qmin, qmax, gmin, gmax, modelnames, models){
  ...
  for (g in 1:(gmax-gmin+1)){
    for (qvalue in 1:(qmax-qmin+1)){
      for (numbmodel in 1:length(run)){
        z<-run[[numbmodel]][[g]][[qvalue]][[1]]$allresults$probaPost
        mapz<-mclust::unmap(run[[numbmodel]][[g]][[qvalue]][[1]]$allresults$clusterlabels)
  ...
  ICLmodel_labels = run[[which(min(ICLvalues) == ICLvalues, arr.ind = T)[3]]][[which(min(ICLvalues) == ICLvalues, arr.ind = T)[1]]][[which(min(ICLvalues) == ICLvalues, arr.ind = T)[2]]][[1]]$allresults$clusterlabels # obtaining model labels
  ICLMessage = NA
...
AIC_function<-function(ll, k, run, qmin, qmax, gmin, gmax, modelnames, models){
  ...
  AICmodel_labels = run[[which(min(AICvalues) == AICvalues, arr.ind = T)[3]]][[which(min(AICvalues) == AICvalues, arr.ind = T)[1]]][[which(min(AICvalues) == AICvalues, arr.ind = T)[2]]][[1]]$allresults$clusterlabels # obtaining model labels
  AICMessage = NA
...
AIC3_function<-function(ll, k, run, qmin, qmax, gmin, gmax, modelnames, models){
  ...
  AIC3model_labels = run[[which(min(AIC3values) == AIC3values, arr.ind = T)[3]]][[which(min(AIC3values) == AIC3values, arr.ind = T)[1]]][[which(min(AIC3values) == AIC3values, arr.ind = T)[2]]][[1]]$allresults$clusterlabels # obtaining model labels
  ...
  for(g in 1:(Gmax-Gmin+1)) {
    ...
    for(qvalue in 1:(qmax-qmin+1)){
      ...
      for (numbmodel in 1:length(allruns)){
        ...
        totaltime[[g]][[qvalue]][[numbmodel]] = allruns[[numbmodel]][[g]][[qvalue]][[1]]$totaltime
        obs[g,qvalue,numbmodel] = nrow(allruns[[numbmodel]][[g]][[qvalue]][[1]]$dataset)
        p = ncol(allruns[[numbmodel]][[g]][[qvalue]][[1]]$dataset)
        
        # calculating log-likelihood
        ll[g,qvalue,numbmodel]<-unlist(tail(allruns[[numbmodel]][[g]][[qvalue]][[1]]$allresults$loglikelihood, n=1)) # save the final log-likelihood
...
downstream_analysis = function(allresults, G, modelnames, q, models){  
  ...
  if((Gmax-Gmin+1) > 10 ) { #NOT WORKING
    ...
    RESULTS <- list(dataset = dataset[[1]][[1]][[1]][[1]]$dataset, 
                    normalization_factors = dataset[[1]][[1]][[1]][[1]]$normalization_factors,
                    dimensionality = ncol(dataset[[1]][[1]][[1]][[1]]$dataset), 
                    gmin = Gmin,
                    gmax = Gmax,
                    qmin = qmin,
                    qmax = qmax,
                    initalization = dataset[[1]][[1]][[1]][[1]]$initalization_method, 
                    allresults = dataset,
                    loglikelihood = ll, 
                    numbofparameters = k,
                    truelabels = dataset[[1]][[1]][[1]][[1]]$truelabels,
    ...
    RESULTS <- list(dataset = allresults[[1]][[1]][[1]][[1]]$dataset,
                    normalization_factors = allresults[[1]][[1]][[1]][[1]]$normalization_factors,
                    dimensionality = ncol(allresults[[1]][[1]][[1]][[1]]$dataset),
                    gmin = Gmin,
                    gmax = Gmax,
                    qmin = qmin,
                    qmax = qmax,
                    initalization = allresults[[1]][[1]][[1]][[1]]$initalization_method,
                    allresults = allresults,
                    loglikelihood = ll, 
                    numbofparameters = k,
                    truelabels = allresults[[1]][[1]][[1]][[1]]$truelabels, 
                    models = c(modelnames[models]),
                    ICL.all = icl,
                    BIC.all = bic,
                    AIC.all = aic,
...
save.image("26Oct2017__nestedparallel_bi2016data.RData")
results2
results4=results4
results6=results6
results8=results8
...
#results
finalresults = downstream_analysis(allresults=toput, G=G_testing, modelnames=modelnames_consider, models=c(2,4,6,8), q=q_testing)
...
g=2
qvalue=2
all(heidel.diag(finalresults$allresults$one[[g]][[qvalue]][[1]]$allresults$loglikelihood, eps=0.1, pvalue=0.05)[,c(1)]==1)
all(heidel.diag(finalresults$allresults$two[[g]][[qvalue]][[1]]$allresults$loglikelihood, eps=0.1, pvalue=0.05)[,c(1)]==1)
all(heidel.diag(finalresults$allresults$three[[g]][[qvalue]][[1]]$allresults$loglikelihood, eps=0.1, pvalue=0.05)[,c(1)]==1)
all(heidel.diag(finalresults$allresults$four[[g]][[qvalue]][[1]]$allresults$loglikelihood, eps=0.1, pvalue=0.05)[,c(1)]==1)

g=3
qvalue=1
plot(finalresults$allresults$one[[g]][[qvalue]][[1]]$allresults$loglikelihood[-1], type="l", xlab="iteration", ylab="logL", main="G=2,q=2,CCU")

#### Checking results ####

# plotting
plot(finalresults$BIC.all$allBICvalues, xaxt = "n", xlab='G,q,M')
...
plot(finalresults$BIC.all$allBICvalues, col=colvector, pch=typevector, main="CCU selection")
...
plot(finalresults$BIC.all$allBICvalues, 
     main= "BIC value vs. the model, G=1:3, q=1:3, M=CCU, CUU, UCU, UUU",
     xlab= "The model (g,q,model)",
     ylab= "BIC value",
     col= colvector+1, pch = colvector, cex = 0.5, lty = "solid", lwd = 5,ylim=c(min(finalresults$BIC.all$allBICvalues)-100,max(finalresults$BIC.all$allBICvalues)+100) )

text(finalresults$BIC.all$allBICvalues, labels=labelvector, cex= 0.8, pos=3)
abline(29872.67,0,lty=2, col=2)
...
g=2
qvalue=2
finalresults$allresults$one[[1]][[g]][[qvalue]][[1]]$allresults$finalmu
finalresults$allresults$one[[1]][[g]][[qvalue]][[1]]$allresults$finalsigma


for (i in 1:1){
  cat("\nRun", i)
  cat("\nAdjusted Rand Index is", adjustedRandIndex(finalresults$truelabels, finalresults$BIC.all$BICmodelselected_labels), "\n")
  print("Mean is ")
  print(finalresults$allresults$one[[1]][[g]][[qvalue]][[1]]$allresults$finalmu)
  print("Sigma is")
  print(finalresults$allresults$one[[1]][[g]][[qvalue]][[1]]$allresults$finalsigma)
}

# ARI 
arindex = list()
for (i in 1:1){
  arindex[[i]] = adjustedRandIndex(finalresults$truelabels, finalresults$BIC.all$BICmodelselected_labels)
}
Reduce("+",arindex)/50 #1
...
bias_function<-function(x, howmanydatasets){ # x = true mean
  ...
  for (d in 1: nrow(truemeans)){ # number of true parameters i.e. cluster size
    ...
    for (j in 1:nrow(x)){ # for the number of clusters
      testing[j]<-norm(t(round(finalresults$allresults$one[[1]][[g]][[qvalue]][[1]]$allresults$finalmu[j,]-y, digits=2)))
    }
    newj<-which(testing == min(testing))
    number<-fakematrix[d,i]
    predicted_mu[number,]<-finalresults$allresults$one[[1]][[g]][[qvalue]][[1]]$allresults$finalmu[newj,]
    }
      
  }  # end of howmanydatasets loop
  return(predicted_mu)
}
predicted_mu<-bias_function(x=truemeans, howmanydatasets=1)

# calculating bias
norm(t(round(predicted_mu[1,]-true_mu11, digits=2)))# 0.73
norm(t(round(predicted_mu[2,]-true_mu21, digits=2))) # 0.58
...
bias_function<-function(x, howmanydatasets){ # x = true mean
  ...
  for (i in 1:howmanydatasets){ # for the number of datasets
      ...
      for (o in 1:clusters){ # number of true parameters i.e. cluster size
        ...
        for (j in 1:clusters){ # for the number of clusters
          testing[j]<-norm(t(round(finalresults$allresults$one[[1]][[g]][[qvalue]][[1]]$allresults$finalsigma[[j]]-y, digits=2)))
        }
        ...
        truesigmaslike[((number-1)*d+1):(number*d),] = finalresults$allresults$one[[1]][[g]][[qvalue]][[1]]$allresults$finalsigma[[o]]
  ...
  results<-list(matrixdata=truesigmaslike, 
                listdata=anotherlist,
                list1 = list1,
                list2 = list2)
  class(results)<-"results"
  return(results)
}
#predicted_sigma = bias_function(x=truesigmas, howmanydatasets=1)

#predicted_sigma11 = Reduce("+",predicted_sigma$list1)/50
#predicted_sigma21 = Reduce("+",predicted_sigma$list2)/50

norm(t(round(finalresults$allresults$one[[1]][[g]][[qvalue]][[1]]$allresults$finalsigma[[1]]-truesigmas[1:6,], digits=2))) # 0.6
norm(t(round(finalresults$allresults$one[[1]][[g]][[qvalue]][[1]]$allresults$finalsigma[[2]]-truesigmas[7:12,], digits=2))) # 0.6
