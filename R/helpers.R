...
model_updates<-function(modelName, zS, ng, z, lambda, isigma, G, pmax.var, Sk, psi) {
  ...
  if (substr(modelName,1,3)=="UUU"){
    ...
    #sigma.var<<-sigma.var
    par<-G*(d*pmax.var-0.5*pmax.var*(pmax.var-1))+G*d
  }
  ...
  if (substr(modelName,1,3)=="UUC"){
    ...
    par<-G*(d*pmax.var-0.5*pmax.var*(pmax.var-1))+G
  }
  ...
  if (substr(modelName,1,3)=="UCU"){
    ...
    par<-G*(d*pmax.var-0.5*pmax.var*(pmax.var-1))+d
  }
  ...
  if (substr(modelName,1,3)=="UCC"){
    ...
    par<-G*(d*pmax.var-0.5*pmax.var*(pmax.var-1))+1
  }
  ...
  if (substr(modelName,1,3)=="CUU"){
    ...
    beta.var<<-beta.var
    bigTheta<<-bigTheta
    for_lam_1<-matrix(rowSums(sapply(1:G,fun_lambda_cuu_1)),d,pmax.var)
    for_lam_2<-list()
    for (k in 1:d){
      k<<-k
      if (pmax.var>1) for_lam_2[[k]]<-solve(matrix(rowSums(sapply(1:G,fun_lambda_cuu_2)),pmax.var,pmax.var)) else for_lam_2[[k]]<-solve(matrix(sum(sapply(1:G,fun_lambda_cuu_2)),pmax.var,pmax.var))
    }
    ...
    psi<-psinew ###This line was missing. So, psi was never updated
    par<-(d*pmax.var-0.5*pmax.var*(pmax.var-1))+G*d
  }
  ...
  if (substr(modelName,1,3)=="CUC"){
    ...
    bigTheta<<-bigTheta
    if (pmax.var>1){
      lambdanew[[1]]<-matrix(rowSums(sapply(1:G,fun_lambda_cuc_1)),d,pmax.var)%*%solve(matrix(rowSums(sapply(1:G,fun_lambda_cuc_2)),pmax.var,pmax.var)	)
    } else lambdanew[[1]]<-matrix(rowSums(sapply(1:G,fun_lambda_cuc_1)),d,pmax.var)%*%solve(matrix(sum(sapply(1:G,fun_lambda_cuc_2)),pmax.var,pmax.var)	)
    ...
    par<-(d*pmax.var-0.5*pmax.var*(pmax.var-1))+G
  }
