# 5 November 2017
# Putting all results from 2 datasets together
...
#### Simulating data data (dataset 1, 6d = works) ####
# Setting 1 G=2, q=2, CUC model
G = 1
q = 3 # always keep as 2
modelnames_testing = "CCU"

N1 = 500 # biological samples e.g. genes
d = 6 # dimensionality e.g. conditions*replicates = total samples 
pi_g1 = c(1) # mixing proportions for G=3

### Mu ###
true_mu11 = c(4,5.5,5,4,4.5,5)  
means1 = rbind(true_mu11)
...
for (model in 1:q){
  if(model==1){
    mat[,model]= runif(d, min=-0.01, max=0.02)
  }
  if(model==2){
    mat[,model]= runif(d, min=-1, max=1)
  }
  if(model==3){
    mat[,model]= runif(d, min=-3, max=3.5)
  }
}
...
lambda[[1]][4:6,]<-(lambda[[1]][4:6,c(3,2,1)]) # if d=6
#lambda[[2]][4:6,]<-(lambda[[2]][4:6,c(2,1)]) # if d=6
...
I_testing = c(1) # total number of datasets to test
...
#  }
save.image("26Jan2018_CCU_g1q3_simdata_allruns_allresults.RData")
...
results26=results6
results28=results8
toput2 = list(one = results22, two=results24, three=results26, four=results28)


#results
finalresults1 = downstream_analysis(allresults=toput, G=G_testing, modelnames=modelnames_consider, models=models_testing, q=q_testing)
finalresults2 = downstream_analysis(allresults=toput2, G=G_testing, modelnames=modelnames_consider, models=models_testing, q=q_testing)
save.image("26Jan2018_UCU_g1q3_simdata_allruns_allresults.RData")
...
g=2
qvalue=2
all(heidel.diag(finalresults1$allresults$one[[1]][[g]][[qvalue]][[1]]$allresults$loglikelihood, eps=0.1, pvalue=0.05)[,c(1)]==1)
all(heidel.diag(finalresults1$allresults$two[[1]][[g]][[qvalue]][[1]]$allresults$loglikelihood, eps=0.1, pvalue=0.05)[,c(1)]==1)
all(heidel.diag(finalresults1$allresults$three[[1]][[g]][[qvalue]][[1]]$allresults$loglikelihood, eps=0.1, pvalue=0.05)[,c(1)]==1)
all(heidel.diag(finalresults1$allresults$four[[1]][[g]][[qvalue]][[1]]$allresults$loglikelihood, eps=0.1, pvalue=0.05)[,c(1)]==1)

g=2
qvalue=2
plot(finalresults1$allresults$three[[1]][[g]][[qvalue]][[1]]$allresults$loglikelihood[-1], type="l", xlab="iteration", ylab="logL", main="G=2,q=2,UCU")

#### Checking results ####

# plotting
plot(finalresults1$BIC.all$allBICvalues, xaxt = "n", xlab='G,q,M')
...
table(finalresults$truelabels,finalresults$BIC.all$BICmodelselected_labels)
...
plot(finalresults1$BIC.all$allBICvalues, col=colvector, pch=typevector, main="CCU selection")
...
plot(finalresults2$BIC.all$allBICvalues, 
     main= "",
     xlab= "The model (g,q,model)",
     ylab= "BIC value",
     col= colvector+1, pch = rep(1:9,4), cex = 1.5, lty = "solid", lwd = 5,ylim=c(min(finalresults2$BIC.all$allBICvalues)-100,max(finalresults2$BIC.all$allBICvalues)+500) )

text(finalresults2$BIC.all$allBICvalues, labels=labelvector, cex= 0.8, pos=3)
abline(28326.82,0,lty=2, col=4)

colvector=c(rep(1,9), rep(2,9), rep(3,9), rep(4,9))
labelvector=c("1,1,1", "1,2,1","1,3,1","2,1,1","2,2,1","2,3,1","3,1,1","3,2,1","3,3,1",   "1,1,2", "1,2,2","1,3,2","2,1,2","2,2,2","2,3,2","3,1,2","3,2,2","3,3,2",   "1,1,3", "1,2,3","1,3,3","2,1,3","2,2,3","2,3,3","3,1,3","3,2,3","3,3,3",   "1,1,4", "1,2,4","1,3,4","2,1,4","2,2,4","2,3,4","3,1,4","3,2,4","3,3,4")
plot(finalresults1$loglikelihood, 
     main= "",
     xlab= "The model (g,q,model)",
     ylab= "LogL value",
     col= colvector+1, pch = colvector, cex = 0.5, lty = "solid", lwd = 5,ylim=c(min(finalresults1$loglikelihood)-100,max(finalresults1$loglikelihood)+100) )

text(finalresults1$loglikelihood, labels=labelvector, cex= 0.8, pos=3)
abline(-13818.92,0,lty=2, col=5)


# checking parameter recovery
g=1
qvalue=3
finalresults1$allresults$three[[1]][[g]][[qvalue]][[1]]$allresults$finalmu
finalresults1$allresults$three[[1]][[g]][[qvalue]][[1]]$allresults$finalsigma
finalresults2$allresults$three[[1]][[g]][[qvalue]][[1]]$allresults$finalmu
finalresults2$allresults$three[[1]][[g]][[qvalue]][[1]]$allresults$finalsigma


# ARI 
arindex_bic = arindex_icl = arindex_aic = arindex_aic3 = list()
datasets=list(one=finalresults1, two=finalresults2)
for (i in 1:1){
...
Reduce("+",arindex_bic)/2 #1
Reduce("+",arindex_icl)/2 #1
Reduce("+",arindex_aic)/2 #1
Reduce("+",arindex_aic3)/2 #1
...
predicted_mu<-bias_function(trueqvalue=3, trueg=1, x=means1, howmanydatasets=1, datasets=list(one=finalresults1), modelnames_testing=modelnames_testing, modelnames_consider=modelnames_consider)

# calculating bias
round(norm(predicted_mu-means1), digits=2) # 3.97
...
predicted_sigma = bias_function(d=6, trueqvalue=3, trueg=1, x=sigmas1, howmanydatasets=1, datasets=list(one=finalresults1), modelnames_testing=modelnames_testing, modelnames_consider=modelnames_consider)

predicted_sigma11 = predicted_sigma$estimatesaslist[[1]]
round(norm(t(predicted_sigma11-sigmas1)), digits=2) # 2.48
