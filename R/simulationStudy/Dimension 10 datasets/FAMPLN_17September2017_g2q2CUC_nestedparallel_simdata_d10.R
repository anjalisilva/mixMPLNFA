...
for (model in 1:q){
  if(model==1){
    mat[,model]= runif(d, min=0.8, max=1)
  }
  if(model==2){
    mat[,model]= runif(d, min=-0.4, max=0)
  }
}
...
lambda[[1]][6:10,]<-(lambda[[1]][6:10,c(2,1)]) # if d=6
...
if (substr(modelnames_testing, 2, 2) == "U"){ # for initializing psi error variance = p*p matrix
  ...
  if (substr(modelnames_testing, 3, 3) == "U"){ #if CUU; for initializing psi isotropic
    for (k in 1:G){
      if(k==1){
        diag(psi[[k]]) = 1:d*(0.4)
      }
  ...
  if (substr(modelnames_testing, 3, 3) == "C"){ #if CUC
    psi[[1]] = psi[[1]] * 0.5
    psi[[2]] = psi[[2]] * 1
    }
  ...
  for(g in 1:(Gmax-Gmin+1)) {
    for(qvalue in 1:(qmax-qmin+1)){
      for (numbmodel in 1:length(models)){
        ...
        if(inputmodel == "CCC"){
          sigma_para[g,qvalue,numbmodel] = ((p*qvalue)-(qvalue*(qvalue-1))/2)+1
        } else if (inputmodel == "CCU"){
          sigma_para[g,qvalue,numbmodel] = ((p*qvalue)-(qvalue*(qvalue-1))/2)+p
        } else if (inputmodel == "CUC") {
          sigma_para[g,qvalue,numbmodel] = ((p*qvalue)-(qvalue*(qvalue-1))/2)+g
        } else if (inputmodel == "CUU") {
          sigma_para[g,qvalue,numbmodel] = ((p*qvalue)-(qvalue*(qvalue-1))/2)+(g*p)
        } else if (inputmodel == "UCC") {
          sigma_para[g,qvalue,numbmodel] = (g*((p*qvalue)-(qvalue*(qvalue-1))/2))+1
        } else if (inputmodel == "UCU") {
          sigma_para[g,qvalue,numbmodel] = (g*((p*qvalue)-(qvalue*(qvalue-1))/2))+p
        } else if (inputmodel == "UUC") {
          sigma_para[g,qvalue,numbmodel] = (g*((p*qvalue)-(qvalue*(qvalue-1))/2))+g
        } else if (inputmodel == "UUU") {
          sigma_para[g,qvalue,numbmodel] = (g*((p*qvalue)-(qvalue*(qvalue-1))/2))+(g*p)
        }
...
G_testing = c(2:2) # total number of cluster sizes to test 
q_testing = c(1:2) # total number of latent factors
models_testing = c(1:8) # total number of models
...
save.image("FAMPLN_17September2017_g2q2CUC_nestedparallel_simdata_d10.RData")
