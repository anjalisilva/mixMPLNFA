...
for (model in 1:q){
  if(model==1){
    mat[,model]= runif(d,  min=0.6, max=0.7)
  }
  if(model==2){
    mat[,model]= runif(d, min=-0.5, max=-0.1)
  }
}
...
q_testing = c(2:2) # total number of latent factors
...
save.image("FAMPLN_17September2017_g2q2CCC_nestedparallel_simdata_d10.RData")
