...
plot(finalresults_1$BIC.all$allBICvalues, ,
     ...
     ylab= "BIC value",
     col= colvector+1, pch = colvector, cex = 0.5, lty = "solid", lwd = 5,ylim=c(min(finalresults_1$BIC.all$allBICvalues)-100,max(finalresults_1$BIC.all$allBICvalues)+100) )
...
cat("\nAdjusted Rand Index is", adjustedRandIndex(finalresults_1$truelabels, finalresults_1$BIC.all$BICmodelselected_labels), "\n")
cat("\nAdjusted Rand Index is", adjustedRandIndex(finalresults_1$truelabels, finalresults_1$ICL.all$ICLmodelselected_labels), "\n")
cat("\nAdjusted Rand Index is", adjustedRandIndex(finalresults_1$truelabels, finalresults_1$AIC.all$AICmodelselected_labels), "\n")
cat("\nAdjusted Rand Index is", adjustedRandIndex(finalresults_1$truelabels, finalresults_1$AIC3.all$AIC3modelselected_labels), "\n")


cat("\nAdjusted Rand Index is", adjustedRandIndex(finalresults_2$truelabels, finalresults_2$BIC.all$BICmodelselected_labels), "\n")
cat("\nAdjusted Rand Index is", adjustedRandIndex(finalresults_2$truelabels, finalresults_2$ICL.all$ICLmodelselected_labels), "\n")
cat("\nAdjusted Rand Index is", adjustedRandIndex(finalresults_2$truelabels, finalresults_2$AIC.all$AICmodelselected_labels), "\n")
cat("\nAdjusted Rand Index is", adjustedRandIndex(finalresults_2$truelabels, finalresults_2$AIC3.all$AIC3modelselected_labels), "\n")



# checking clustering of labels
table(finalresults_1$truelabels, finalresults_1$BIC.all$BICmodelselected_labels)
table(finalresults_2$truelabels, finalresults_2$BIC.all$BICmodelselected_labels)
...
bias_function<-function(trueqvalue, trueg, x, howmanydatasets, datasets, modelnames_testing, modelnames_consider){ # x = true mean
  ...
  fakematrix<-matrix(c(1:(nrow(x)*howmanydatasets)), ncol=howmanydatasets, nrow=nrow(truemeans), byrow = TRUE)
  for (d in 1: nrow(truemeans)){ # number of true parameters i.e. cluster size
    y<-x[d,]
    for (i in 1:howmanydatasets){ # for the number of datasets
      testing<-vector()
      for (j in 1:nrow(x)){ # for the number of clusters
        testing[j]<-norm(t(round(datasets[[i]]$allresults[[grep(modelnames_testing,modelnames_consider)]][[1]][[trueg]][[trueqvalue]][[1]]$allresults$finalmu[j,]-y, digits=2)))
      }
      newj<-which(testing == min(testing))
      number<-fakematrix[d,i]
      predicted_mu[number,]<-datasets[[i]]$allresults[[grep(modelnames_testing,modelnames_consider)]][[1]][[trueg]][[trueqvalue]][[1]]$allresults$finalmu[newj,]
    }
...
# calculating bias
norm(t(round(colMeans(predicted_mu[1:2,])-true_mu11, digits=2)))# 1.76
norm(t(round(colMeans(predicted_mu[3:4,])-true_mu21, digits=2))) # 2.18
  ...
  for (i in 1:howmanydatasets){ # for the number of datasets
      ...
      for (o in 1:clusters){ # number of true parameters i.e. cluster size
        ...
        for (j in 1:clusters){ # for the number of clusters
          testing[j]<-norm(t(round(datasets[[i]]$allresults[[grep(modelnames_testing,modelnames_consider)]][[1]][[trueg]][[trueqvalue]][[1]]$allresults$finalsigma[[j]]-y, digits=2)))
        }
        ...
        truesigmaslike[((number-1)*d+1):(number*d),] = datasets[[i]]$allresults[[grep(modelnames_testing,modelnames_consider)]][[1]][[trueg]][[trueqvalue]][[1]]$allresults$finalsigma[[o]]
...
predicted_sigma = bias_function(d=10, trueqvalue=2, trueg=2, x=sigmas1, howmanydatasets=2, datasets=list(one=finalresults_1, two=finalresults_2), modelnames_testing=modelnames_testing, modelnames_consider=modelnames_consider)

predicted_sigma11 = Reduce("+",predicted_sigma$list1)/2
predicted_sigma21 = Reduce("+",predicted_sigma$list2)/2

norm(t(round(predicted_sigma11-truesigmas[1:10,], digits=2))) #  2
norm(t(round(predicted_sigma21-truesigmas[11:20,], digits=2))) # 2.18
save.image("27Oct2017_CCU_simdata_allruns_results.RData")
