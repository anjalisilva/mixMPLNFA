# 4 Nov 2017
# testing CUU model for G=1:3, q=1:3
...
#### Simulating data data (dataset 1, 10d = works) ####
...
results16=results2
results18=results2
toput = list(one = results12, two=results14, three=results16, four=results18)
#results
...
save.image("4Nov2017_CUU_simdata_allruns_allresults_d10.RData")
...
g=2
qvalue=2
plot(finalresults_1$allresults$one[[1]][[g]][[qvalue]][[1]]$allresults$loglikelihood[-1], type="l", xlab="iteration", ylab="logL", main="G=2,q=2,CUU")
plot(finalresults_2$allresults$one[[1]][[g]][[qvalue]][[1]]$allresults$loglikelihood[-1], type="l", xlab="iteration", ylab="logL", main="G=2,q=2,CUU")
...
plot(finalresults_2$BIC.all$allBICvalues, 
     main= "",
     xlab= "The model (g,q,model)",
     ylab= "BIC value",
     col= colvector+1, pch = rep(1:9,4), cex = 1.5, lty = "solid", lwd = 5,ylim=c(min(finalresults_2$BIC.all$allBICvalues)-200,max(finalresults_2$BIC.all$allBICvalues)+300) )

text(finalresults_2$BIC.all$allBICvalues, labels=labelvector, cex= 0.8, pos=3)
abline(44063.40,0,lty=2, col=3)
...
plot(finalresults_1$loglikelihood, 
     ...
     xlab= "The model (g,q,model)",
     ylab= "LogL value",
     col= colvector+1, pch = rep(1:9,4), cex = 1.5, lty = "solid", lwd = 5,ylim=c(min(finalresults_1$loglikelihood)-100,max(finalresults_1$loglikelihood)+100) )
...
# calculating bias
round(norm(t(colMeans(predicted_mu[1:2,])-means1[1,])), digits=2)#  2.71
round(norm(t(colMeans(predicted_mu[3:4,])-means1[2,])), digits=2) # 3.03
...
predicted_sigma11 = round(Reduce("+",predicted_sigma$list1)/2,2)
predicted_sigma21 = round(Reduce("+",predicted_sigma$list2)/2,2)

round(norm(t(predicted_sigma11-sigmas1[11:20,])), digits=2) # 4.6
round(norm(t(predicted_sigma21-sigmas1[1:10,])), digits=2) # 5
