...
[Subedi and Browne (2020)](https://doi.org/10.1002/sta4.310) had proposed a framework for parameter estimation utilizing variational Gaussian approximation (VGA) for mixtures of multivariate Poisson- log normal distribution-based mixture models. 
Markov chain Monte Carlo expectation-maximization (MCMC-EM) has also been used for parameter estimation of MPLN-based mixture models, but VGA was shown to be computationally efficient (Silva et al., 2023). This method is employed in the function *MPLNFAClust*. Variational approximations (Wainwright et al., 2008) are approximate inference techniques in which a computationally convenient approximating density is used in place of a more complex but 'true' posterior density. The approximating density is obtained by minimizing the Kullback-Leibler (KL) divergence between the true and the approximating densities. The VGAs approach is computationally efficient, however, it does not guarantee an exact posterior (Ghahramani and Beal, 1999). 
...
``` r
nrow(sampleData$dataset) # 2000 observations
ncol(sampleData$dataset) # 9 dimensions
class(sampleData$dataset) # matrix 
typeof(sampleData$dataset) # integer
summary(sampleData$dataset) # summary of data
dim(sampleData$dataset) # 2000 x 9
```
...
## Clustering
<div style="text-align:left">
Once the count data is available, clustering can be performed using the *mixMPLNFA::MPLNFA* function. See *?mixMPLNFA::MPLNFA* for more information, including examples and references. Here, clustering will be performed using the above generated dataset. 

Below, clustering of *sampleData$dataset* is performed for g = 1:3 with latent factors from p = 1:3 with model tested including CCU and UUU. 
...
table(MPLNFAResults$BICresults$BICmodelSelectedLabels, sampleData$trueMembership)
#     1    2    3
#1    0    0 1000
#2    0  594    0
#3  406    0    0
...
MPLNLineColor <- mixMPLNFA::mplnFAVisLine(
                    dataset = sampleData$dataset,
                    clusterMembershipVector = MPLNFAResults$BICresults$BICmodelSelectedLabels,
...
``` r
par(mfrow = c(1, 2))
graphics::matplot(MPLNFAResults$logLikelihood, xlab = "Run",
                  ylab = "logL", type = c("b"), pch = 1, lty = 2, xaxt = "n") 
                  
numberVec <- c(1:((MPLNFAResults$gmax - MPLNFAResults$gmin + 1) * (MPLNFAResults$pmax - MPLNFAResults$pmin + 1) * length(MPLNFAResults$   )))

                  
graphics::axis(1, xaxp = c(min(MPLNFAResults$BICresults$BICmodelSelectedLabels), 
               max(MPLNFAResults$BICresults$BICmodelSelectedLabels), 1), 
               las = 2)
               
ICvalues <- matrix(c(MPLNFAResults$BICresults$allBICvalues,   
              MPLNFAResults$ICLresults$allICLvalues,
              MPLNFAResults$AICresults$allAICvalues,
              MPLNFAResults$AIC3results$allAIC3values),
              ncol=4) 
graphics::matplot(ICvalues, xlab = "Run", ylab = "Information criteria value", 
                  type = c("b"), pch = 1, col = 1:4, xaxt="n") 
graphics::axis(1, xaxp = c(min(MPLNFAResults$BICresults$allBICvalues)-10000, 
               max(MPLNFAResults$BICresults$allBICvalues) +10000, 1), 
               las=2)
